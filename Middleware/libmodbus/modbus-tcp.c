...
static int _modbus_tcp_build_request_basis(modbus_t* ctx, int function, int addr, int nb,
                                           uint8_t* req) {
...
static int _modbus_tcp_pre_check_confirmation(modbus_t* ctx, const uint8_t* req, const uint8_t* rsp,
                                              int rsp_length) {
...
//     if(ctx->backend_data) {
//         modbus_tcp_pi_t* ctx_tcp_pi = ctx->backend_data;
//         vPortFree(ctx_tcp_pi->node);
//         vPortFree(ctx_tcp_pi->service);
//         vPortFree(ctx->backend_data);
//     }
